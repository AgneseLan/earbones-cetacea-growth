...
ordination_scores <- read.table("Data/_gpsa_ordination_projections_periotic_L.RData")
...
ordination_values <- read.table("Data/_gpsa_ordination_values_periotic_L.RData")
...
raw_dat$code <- factor(raw_dat$code, #check levels below from string printed with the code above
                       levels = c("Bf1",   "Hf3"  , "Mf1" ,  "Sf2"  , "Lagf1","Lagf2", "Monf1" ,"Phof1" ,"Af3" ,  "Af4" , "Af5" ,  "Af6",  
...
save.image("D:/Agnese/ear bones project/R/periotic_R/.RData")
savehistory("D:/Agnese/ear bones project/R/periotic_R/.Rhistory")
...
PCA_all_ggplot <- ggplot(pcscores_all, aes(x = Comp1, y = Comp2, label = specimens, colour = stage, shape = group))+
  ...
  theme_bw()+
  xlab("PC 1 (17.68%)")+ #copy this from standard PCA plot (PCA_all_plot)
  ylab("PC 2 (10.59%)")+
...
pcscores_all_earlyfetus <- pcscores_all_stages[[1]]
pcscores_all_latefetus <- pcscores_all_stages[[2]]
pcscores_all_postnatal <- pcscores_all_stages[[3]]
...
#Create one tibble for each stage
#First split between stages
pcscores_all_stages <- pcscores_all %>% group_by(stage) %>% group_split()


##Regression PC1 and PC2 vs periotic length - check if size an important factor in results overall
...
##Evaluate allometry and get the allometry-free shapes using ln periotic length
...
allometry_ggplot <- ggplot(allometry_plot, aes(x = size, y = RegScores, label = specimens, colour = stage, shape = group))+
  ...
  xlab("Ln(Periotic length)")+
  ylab("Regression Score")+
  ggtitle ("Allometry plot - p-value = 0.003**")+  #copy from model summary
  theme(plot.title = element_text(face = "bold", hjust = 0.5))+
...
ordination_scores_ggplot <- ggplot(ordination_scores, aes(x = axis1, y = axis2, label = specimens, colour = stage, shape = group))+
  ...
  scale_shape_manual(name = "Group", labels = c("Mysticeti", "Odontoceti"), values = shapes)+
  theme_bw()+
  xlab("I (17.68%)")+ #copy this from ordination values_100 column printed before
  ylab("II (10.59%)")+
  ggtitle("PCOORD GPSA")+
...
allometry_group_anova <- procD.lm(coords ~ periotic_log * group, iter = 999, data = gdf, RRPP = F)
...
allometry_add_group_anova <- procD.lm(coords ~ periotic_log + group, iter = 999, data = gdf, RRPP = F)
...
gdf_earlyfetus <- geomorph.data.frame(coords = shape_array[,,1:6], 
                                      code = classifiers$code[1:6], group = classifiers$group[1:6], 
                                      stage = classifiers$stage[1:6], category = classifiers$category[1:6],
                                      bulla_log = classifiers$bullaL_log[1:6], periotic_log = classifiers$perioticL_log[1:6])
glimpse(gdf_earlyfetus)

gdf_latefetus <- geomorph.data.frame(coords = shape_array[,,7:18], 
                                     code = classifiers$code[7:18], group = classifiers$group[7:18], 
                                     stage = classifiers$stage[7:18], category = classifiers$category[7:18],
                                     bulla_log = classifiers$bullaL_log[7:18], periotic_log = classifiers$perioticL_log[7:18])
glimpse(gdf_latefetus)

gdf_postnatal <- geomorph.data.frame(coords = shape_array[,,19:29], 
                                     code = classifiers$code[19:29], group = classifiers$group[19:29], 
                                     stage = classifiers$stage[19:29], category = classifiers$category[19:29],
                                     bulla_log = classifiers$bullaL_log[19:29], periotic_log = classifiers$perioticL_log[19:29])
glimpse(gdf_postnatal)

#Make separate dataframe to check shape is not different postnatal in odontocetes
gdf_postnatal_odont <- geomorph.data.frame(coords = shape_array[,,22:29], 
                                           code = classifiers$code[22:29], group = classifiers$group[22:29], 
                                           stage = classifiers$stage[22:29], category = classifiers$category[22:29],
                                           bulla_log = classifiers$bullaL_log[22:29], periotic_log = classifiers$perioticL_log[22:29])
...
#Early fetus
pcscores_group_earlyfetus_anova <- procD.lm(pcscores_all_earlyfetus[1:29] #this number is the number of Components from the PCA, make sure it is correct by calling PCA_all$x
...
#Late fetus
pcscores_group_latefetus_anova <- procD.lm(pcscores_all_latefetus[1:29] #this number is the number of Components from the PCA, make sure it is correct by calling PCA_all$x
